...
print("Using nodes %s:" % (nodes.keys()), file=sys.stderr)
...
try:
    while True:
        ...
        for node_id, node in nodes.iteritems():
            if ((r.hget('commands:node:%d'%node_id, 'power_snap_relay_ctrl_trig')) == 'True'):
                while ((time.time() - float(r.hget('throttle:node:%d'%node_id,'last_command_sec'))) < cmd_time_sec):
                    ...
                    time.sleep(.1)
                #print("Sent!")
                if ((r.hget('commands:node:%d'%node_id, 'power_snap_relay_cmd')) == 'on'):
            ...
            if ((r.hget('commands:node:%d'%node_id, 'power_snap_0_ctrl_trig')) == 'True'):
                while ((time.time() - float(r.hget('throttle:node:%d'%node_id,'last_command_sec'))) < cmd_time_sec):
                    #print('Command sent too soon, waiting 100ms and trying again...')
                    time.sleep(.1)
                #print("Sent!")
                if (r.hget('commands:node:%d'%node_id, 'power_snap_0_cmd') == 'on'):
            ...
            if ((r.hget('commands:node:%d'%node_id, 'power_snap_1_ctrl_trig')) == 'True'):
                while ((time.time() - float(r.hget('throttle:node:%d'%node_id,'last_command_sec'))) < cmd_time_sec):
                    #print('Command sent too soon, waiting 100ms and trying again...')
                    time.sleep(.1)
                #print("Sent!")
                if (r.hget('commands:node:%d'%node_id, 'power_snap_1_cmd') == 'on'):
            ...
            if ((r.hget('commands:node:%d'%node_id, 'power_snap_2_ctrl_trig')) == 'True'):
                while ((time.time() - float(r.hget('throttle:node:%d'%node_id,'last_command_sec'))) < cmd_time_sec):
                    #print('Command sent too soon, waiting 100ms and trying again...')
                    time.sleep(.1)
                #print("Sent!")
                if (r.hget('commands:node:%d'%node_id, 'power_snap_2_cmd') == 'on'):
            ...
            if ((r.hget('commands:node:%d'%node_id, 'power_snap_3_ctrl_trig')) == 'True'):
                while ((time.time() - float(r.hget('throttle:node:%d'%node_id,'last_command_sec'))) < cmd_time_sec):
                    #print('Command sent too soon, waiting 100ms and trying again...')
                    time.sleep(.1)
                #print("Sent!")
                if (r.hget('commands:node:%d'%node_id, 'power_snap_3_cmd') == 'on'):
            ...
            if (r.hget('commands:node:%d'%node_id, 'power_fem_ctrl_trig') == 'True'):
                while ((time.time() - float(r.hget('throttle:node:%d'%node_id,'last_command_sec'))) < cmd_time_sec):
                    #print('Command sent too soon, waiting 100ms and trying again...')
                    time.sleep(.1)
                #print("Sent!")
            ...
            if (r.hget('commands:node:%d'%node_id, 'power_pam_ctrl_trig') == 'True'):
                while ((time.time() - float(r.hget('throttle:node:%d'%node_id,'last_command_sec'))) < cmd_time_sec):
                    #print('Command sent too soon, waiting 100ms and trying again...')
                    time.sleep(.1)
                #print("Sent!")
                if (r.hget('commands:node:%d'%node_id, 'power_pam_cmd') == 'on'):
                    node.power_pam('on')
                else:
                    node.power_pam('off')
