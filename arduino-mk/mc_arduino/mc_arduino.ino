...
// Note that some pins are active HIGH and others active LOW
...
#define PAM_ON HIGH
#define PAM_OFF LOW
#define SNAPv2_0_ON LOW
#define SNAPv2_0_OFF HIGH
#define SNAPv2_1_ON LOW
#define SNAPv2_1_OFF HIGH
#define SNAPv2_2_ON LOW
#define SNAPv2_2_OFF HIGH 
#define SNAPv2_3_ON LOW
#define SNAPv2_3_OFF HIGH 
...
String command; // Stores the contents of the command send by user

unsigned int EEPROM_SIZE = 1024;
unsigned int eeadrMac = 0;

// I2C digital i/o board that assigns node ID
const byte SX1509_ADDRESS = 0x3E;
SX1509 io;

// Used in statusStruct packet timing 
...
struct status {
	...
	float mcpTempBot = -99;             // Mid temperature sensor value
	...
	bool  snapv2_0 = false;           // Power status of SNAP 0 and 1
	bool  snapv2_1 = false;           // Power status of SNAP 0 and 1
	bool  snapv2_2 = false;           // Power status of SNAP 2 and 3
	bool  snapv2_3 = false;           // Power status of SNAP 2 and 3
...
void setup() {
	...
	// SNAPv2_0_PIN: Active LOW so HIGH is off
	digitalWrite(SNAPv2_0_PIN, SNAPv2_0_OFF);
	pinMode(SNAPv2_0_PIN, OUTPUT);
	digitalWrite(SNAPv2_0_PIN, SNAPv2_0_OFF);

	// SNAPv2_1_PIN: Active LOW so HIGH is off
	digitalWrite(SNAPv2_1_PIN, SNAPv2_1_OFF);
	pinMode(SNAPv2_1_PIN, OUTPUT);
	digitalWrite(SNAPv2_1_PIN, SNAPv2_1_OFF);

	// SNAPv2_2_PIN: Active LOW so HIGH is off
	digitalWrite(SNAPv2_2_PIN, SNAPv2_2_OFF);
	pinMode(SNAPv2_2_PIN, OUTPUT);
	digitalWrite(SNAPv2_2_PIN, SNAPv2_2_OFF);

	// SNAPv2_3_PIN: Active LOW so HIGH is off
	digitalWrite(SNAPv2_3_PIN, SNAPv2_3_OFF);
	pinMode(SNAPv2_3_PIN, OUTPUT);
	digitalWrite(SNAPv2_3_PIN, SNAPv2_3_OFF);
...
void loop() {
	...
	// Find bottom temp sensor and take read value
	if (mcpBot.begin(TEMP_BOT_ADDR)) {
		statusStruct.mcpTempBot = mcpBot.readTempC();
	}
	else {
	...
	// If it's been more than 2 seconds since last UDP packet transfer, send another one
	unsigned long time_now = millis();
	if ((time_now - last_sent) > 2000) {
		serialUdp(String(time_now - last_sent));
	...
	// Renew DHCP lease - not sure if necessary since Arduino IPs are static in dnsmasq.conf
	Ethernet.maintain();
